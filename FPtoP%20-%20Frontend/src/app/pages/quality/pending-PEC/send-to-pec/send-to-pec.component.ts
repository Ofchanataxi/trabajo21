...
@Component({
  selector: "app-send-to-pec",
  templateUrl: "./send-to-pec.component.html",
  styleUrls: ["./send-to-pec.component.scss"],
})
export class SendToPECComponent {
  items: any[] = [];
  ...
  ngOnInit(): void {
    this.subscription = this.apiService
      .getData(
        ...
        environment.endpoints.getPendingWells.name
      )
      .subscribe({
        next: (response: any[]) => {
          let arr = [];
  ...
  toggleButton(id: number, param: string) {
    this.router.navigate(["/quality/pending-send/upload-files"], {
