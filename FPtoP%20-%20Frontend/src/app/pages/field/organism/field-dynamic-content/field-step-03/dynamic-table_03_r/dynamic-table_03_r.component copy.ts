...
returnQuantityToRelease(row: any){
  ...
  const rowsToUpdate = this.groupedData[row.type].filter((r: any) => r['idElementRelease'] === groupId )
  console.log('rowsToUpdate',rowsToUpdate)
  
    rowsToUpdate.forEach(r => {
        r['ElementRelease.availablequantity']= val
        if (r.group === row.group ) {r['ElementTally.quantity'] = 0} 
      ...
      //const rowsToUpdate = this.groupedData[row.type]
      const rowsToUpdate = this.groupedData[row.type]
  .filter((r: any) => r.idElementRelease === row.idElementRelease );  
  console.log('rowsToUpdate new',rowsToUpdate)
  rowsToUpdate.forEach(r => {
      r['ElementRelease.availablequantity']= val
      ...
      setTimeout(() => this.rowUpdated.emit(row), 0);  
      const rowsToUpdateRight = this.groupedData[row.type]
      .filter((r: any) => r.group === row.group );  
      rowsToUpdateRight.forEach(r => {
          console.log('new update tally quantity', row.group)
          //r['ElementRelease.availablequantity']= val
          r['ElementTally.quantity'] = 0 
          this.onInputChange(r, 'ElementTally.quantity')
          //this.onInputChange(r, 'ElementRelease.availablequantity')     
          });
          const indicesToRemove = this.updatedFields.reduce((acc: number[], r: any, index) => {
            if (r.group === row.group) acc.push(index);
            return acc;
          }, []);

          for (let i = indicesToRemove.length - 1; i >= 0; i--) {
            this.updatedFields.splice(indicesToRemove[i], 1);
          }
