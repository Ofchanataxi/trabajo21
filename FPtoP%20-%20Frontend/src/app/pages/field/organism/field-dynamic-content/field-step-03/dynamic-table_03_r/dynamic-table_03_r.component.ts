...
@Component({
  selector: "app-dynamic-table_03_r",
  templateUrl: "./dynamic-table_03_r.component.html",
  styleUrls: ["./dynamic-table_03_r.component.scss"],
})
export class DynamicTableComponent03r implements OnChanges {
...
>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.groupedData || changes.searchTerm) {
      ...
      //this.index.emit(this.maxRowIndex);
      this.calculateShouldDisplayColumn();
      this.updateRowsInGroupCount();
...
onDrop(event: CdkDragDrop<any[]>, group: string) {
  const rows = this.dataToDisplay[group];
  const draggedRow = event.item.data;
  const draggedIndex = event.previousIndex;
  let dropIndex = event.currentIndex;

  const groupKey = draggedRow.group;
  const isGrouped = draggedRow.rowsInGroup && draggedRow.rowsInGroup > 1;

  // Buscar índices del grupo
  const groupIndexes = rows
    .map((r, i) => ({ row: r, index: i }))
    .filter(x => x.row.group === groupKey)
    .map(x => x.index);

  const groupStart = Math.min(...groupIndexes);
  const groupEnd = Math.max(...groupIndexes);
  const isDraggingGroup = isGrouped && groupIndexes.includes(draggedIndex);

  // ✅ Caso especial: reordenar dentro del mismo grupo
  const isInternalReorder = isDraggingGroup && dropIndex >= groupStart && dropIndex <= groupEnd;
  if (isInternalReorder) {
    const from = groupIndexes.indexOf(draggedIndex);
    const to = groupIndexes.indexOf(dropIndex);
    if (from === -1 || to === -1) return;

    const groupRows = groupIndexes.map(i => rows[i]);
    moveItemInArray(groupRows, from, to);
    groupIndexes.forEach((idx, i) => {
      rows[idx] = groupRows[i];
    });
  ...
  // 🧪 Simular movimiento para validación
  const clonedRows = [...rows];
  if (isDraggingGroup) {
    const groupRows = groupIndexes.map(i => clonedRows[i]);
    clonedRows.splice(groupStart, groupRows.length);

    // ⚠️ Mover al final correctamente
    let adjustedDropIndex = dropIndex;
    if (dropIndex > groupStart) {
      adjustedDropIndex = dropIndex + groupRows.length;
    }
    if (adjustedDropIndex >= clonedRows.length) {
      adjustedDropIndex = clonedRows.length;
    }

    clonedRows.splice(adjustedDropIndex, 0, ...groupRows);
  } else {
    moveItemInArray(clonedRows, draggedIndex, dropIndex);
  }

  // 🔍 Validar que no se rompan los grupos
  const groupMap: { [groupId: string]: number[] } = {};
  clonedRows.forEach((row, index) => {
    if (row.group && row.rowsInGroup > 1) {
      if (!groupMap[row.group]) groupMap[row.group] = [];
      groupMap[row.group].push(index);
    }
  });

  const gruposInvalidos = Object.entries(groupMap).filter(([_, indices]) => {
  ...
  if (gruposInvalidos.length > 0) {
    console.warn("Drop cancelado: rompería uno o más grupos", gruposInvalidos);
    return;
  }
  ...
  if (isDraggingGroup) {
    const groupRows = groupIndexes.map(i => rows[i]);
    rows.splice(groupStart, groupRows.length);

    let adjustedDropIndex = dropIndex;
    if (dropIndex > groupStart) {
      adjustedDropIndex = dropIndex - groupRows.length+1;
    }
    if (adjustedDropIndex >= rows.length) {
      adjustedDropIndex = rows.length;
    }

    rows.splice(adjustedDropIndex, 0, ...groupRows);
  } else {
    moveItemInArray(rows, draggedIndex, dropIndex);
  }
...
buildConnectionMapFrom(rows: any[]): (number | null)[] {
  const map: (number | null)[] = [];
  const groupTracker: { [key: string]: number } = {};

  rows.forEach((row, index) => {
    const groupKey = row.group;
    if (!groupKey) {
      map.push(null); // sin conexión
      return;
    }

    if (!(groupKey in groupTracker)) {
      groupTracker[groupKey] = index;
      map.push(null); // líder del grupo
    } else {
      map.push(groupTracker[groupKey]); // conectado al líder
    }
  });
...
onValidateMax(value: number, row: any): void {
  const max = row['ElementRelease.quantity'];
  const min = 0;
...
countRowsInGroup(row: any, group: string): number {
  ...
  .filter((r: any) => r.group === row['group'] && r.tallyGroupParent == row.tallyGroupParent && r.origin===row?.orign  );
  console.log('firstIndexOfGroup', firstIndexOfGroup, firstIndexOfGroup.length) 
...
transferRowgroup(row: any, increase: string, decrease: string, groupid: string){
  ...
  .filter((r: any) => r.group === groupId);  
  rowsToUpdate.forEach(r => {
    console.log('transferRowgroup', r.group, r.tallyGroupParent, r.groupsdisplaybutton)
    this.transferRow(r,increase, decrease, groupid)
  });
  console.log('rowsToUpdate',rowsToUpdate)
}

returnQuantityToRelease(row: any){
  const groupId = row.idElementRelease;
  const val = row['ElementRelease.availablequantity'] + row['ElementTally.quantity']
  
  if (row['origin']!='new'){
    
  const allRows: any[] = this.groupedData[row.type] ?? [];

// 1. Find siblings = same groupstallyrighttable
const siblings = allRows.filter(r => r?.groupstallyrighttable === row?.groupstallyrighttable);

// 2. Extract all idElementRelease from siblings
const releaseIds = new Set(
  siblings.map(r => r?.idElementRelease).filter(id => id !== null && id !== undefined)
);

// 3. Update all rows that match those releaseIds
const rowsToUpdate = allRows.filter(r => releaseIds.has(r?.idElementRelease));

console.log('rowsToUpdate based on releaseIds', rowsToUpdate);

rowsToUpdate.forEach(r => {
  r['ElementRelease.availablequantity'] = val;

  // Set quantity = 0 only if it's in the same visual group
...
setTimeout(() => this.rowUpdated.emit(row), 0);
      ...
      const allRows: any[] = ([] as any[]).concat(...Object.values(this.groupedData));

    // 2) Hermanos: mismos groupstallyrighttable
    const siblings = allRows.filter(r => r?.groupstallyrighttable === row?.groupstallyrighttable);

    // 3) Set de idElementRelease de los hermanos
    const releaseIds = new Set(
      siblings
        .map(r => r?.idElementRelease)
    ...
    // 4) Actualizar todos los elementos que compartan cualquiera de esos idElementRelease
    Object.keys(this.groupedData).forEach(typeKey => {
      this.groupedData[typeKey].forEach((r: any) => {
        if (releaseIds.has(r?.idElementRelease)) {
          r['ElementRelease.availablequantity'] = val;
          this.onInputChange(r, 'ElementRelease.availablequantity');
        }
      });
    ...
    // 5) (Opcional) Resetear tally para los que comparten el mismo groupstallyrighttable que el row
    Object.keys(this.groupedData).forEach(typeKey => {
      this.groupedData[typeKey].forEach((r: any) => {
        if (r?.groupstallyrighttable === row?.groupstallyrighttable) {
          r['ElementTally.quantity'] = 0;
          this.onInputChange(r, 'ElementTally.quantity');
        }
      });
    ...
    // 6) Limpiar updatedFields (por idElementRelease o por groupstallyrighttable)
    if (Array.isArray(this.updatedFields)) {
      for (let i = this.updatedFields.length - 1; i >= 0; i--) {
        const uf: any = this.updatedFields[i];
        if (releaseIds.has(uf?.idElementRelease) || uf?.groupstallyrighttable === row?.groupstallyrighttable) {
...
calculateShouldDisplayColumn(): void {
  const partitionGroups: { [key: string]: any[] } = {};

  // Paso 1: Agrupar por partitionKey
  Object.values(this.groupedData).forEach((group: any[]) => {
    group.forEach(row => {
      const key = `${row.tallyGroupParent == null ? 1 : 2}${row.group}`;

      if (!partitionGroups[key]) {
        partitionGroups[key] = [];
      }
      partitionGroups[key].push(row);
    });
  });

  // Paso 2: Ordenar cada grupo por sequence_number y marcar el primero
  Object.values(partitionGroups).forEach((rows: any[]) => {
    const sorted = rows
      .filter(r => r['ElementTally.sequence_number'] != null)
      .sort((a, b) => Number(a['ElementTally.sequence_number']) - Number(b['ElementTally.sequence_number']));

    // Marcar todos como false por defecto
    rows.forEach(r => r.shouldDisplay = false);

    if (sorted.length > 0) {
      sorted[0].shouldDisplay = true;
    }
  });
}

updateRowsInGroupCount(): void {
  const groupMap: { [key: string]: any[] } = {};

  // Step 1: Group all rows by composite key
  Object.values(this.groupedData).forEach((group: any[]) => {
    group.forEach(row => {
      const key = `${row.group}|${row.tallyGroupParent}|${row.origin}`;
      if (!groupMap[key]) {
        groupMap[key] = [];
      }
  ...
  // Step 2: Assign the count to each row in the group
  Object.values(groupMap).forEach(rows => {
    const count = rows.length;
    rows.forEach(row => {
      row.rowsInGroup = count;
    });
  });
}

buildConnectionMap(): void {
  this.connectionMap = [];

  Object.values(this.dataToDisplay).forEach((rows: any[]) => {
    const groupTracker: { [key: string]: number } = {};

    rows.forEach((row, index) => {
      const groupKey = row.group;
      if (!groupKey) {
        this.connectionMap.push(null); // sin grupo
        return;
      }

      if (!(groupKey in groupTracker)) {
        groupTracker[groupKey] = index;
        this.connectionMap.push(null); // líder del grupo
      } else {
        this.connectionMap.push(groupTracker[groupKey]); // conectado a su líder
      }
    });
  });
}
...
function isFunction(component: DynamicTableComponent03r, methodName: any): methodName is keyof DynamicTableComponent03r {
  return typeof component[methodName as keyof DynamicTableComponent03r] === 'function';
}
