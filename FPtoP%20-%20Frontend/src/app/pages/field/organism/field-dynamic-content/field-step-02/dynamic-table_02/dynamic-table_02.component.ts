...
@Component({
  selector: "app-dynamic-table_02",
  templateUrl: "./dynamic-table_02.component.html",
  styleUrls: ["./dynamic-table_02.component.scss"],
})
export class DynamicTableComponent02 implements OnChanges {
...
>();
  ...
  onRejectConfirmed(comment: any):any{
    ...
    this.rejectedEquipmentChange.emit(this.workingRow);
    this.prepareDataForDisplay();
...
shouldShowGroupButtons(row: any, groupId: string): boolean {
  const groupRows = [...this.dataToDisplay[row.type]]
...
countRowsInGroup(row: any, group: string): number {
  ...
  const firstIndexOfGroup = this.dataToDisplay[row.type]
  .filter((r: any) => r.groups === groupId);
  return firstIndexOfGroup.length;
}
trueFalseSetterGroup(row: any, val: boolean) {
  const groupId = row.groups;
  ...
  openOverlay(row: any){
    this.workingRow = row
    this.genericRequestPage.openOverlay()
  }
...
function isFunction(component: DynamicTableComponent02, methodName: any): methodName is keyof DynamicTableComponent02 {
  return typeof component[methodName as keyof DynamicTableComponent02] === 'function';
}
