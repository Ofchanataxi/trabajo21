...
@Component({
  selector: "app-dynamic-table_05",
  templateUrl: "./dynamic-table_05.component.html",
  styleUrls: ["./dynamic-table_05.component.scss"],
})
export class DynamicTableComponent05 implements OnChanges {
...
>();
  ...
  prepareDataForDisplay() {
    ...
    //console.log('getCumulativeValuesUpToRow 0',this.dataToDisplay)
  }
      ...
      filteredResult[groupKey] = data[groupKey].filter((row: any) => {
        return Object.keys(this.columnConfig.columns).some((key) => {
          ...
          if (column.filterable) {
            console.log('search key', groupKey,key,this.searchTerm,row[key],  row[key]
              ?.toString()
              .toLowerCase()
  ...
  for (let i = 0; i <= rowIndex; i++) {
    ...
    labels.forEach((label, idx) => {
      ...
      cumulative[idx] += val;
      //console.log('getCumulativeValuesUpToRow 1',idx, val)
      row['cumulativeCalcValue']=cumulative[idx] 
      //console.log("row['cumulativeCalcValue']", row['cumulativeCalcValue'], row)
    });
  }
  //console.log('getCumulativeValuesUpToRow 2',labels)

  return cumulative;
}
private computeCumulativeValues() {
  Object.keys(this.dataToDisplay).forEach((groupKey) => {
    const rows = this.dataToDisplay[groupKey];
    const labels = this.getColumnLabel('medidatotal')?.split('/') || ['medidatotal'];
    ...
    rows.forEach((row) => {
      labels.forEach((label, idx) => {
        const val = Number(row?.[label]) || 0;
        cumulative[idx] = parseFloat((cumulative[idx] + val).toFixed(2));
      });
      // store a rounded copy
      row['cumulativeCalcValue'] = cumulative.map(v => parseFloat(v.toFixed(2)));
    });
  });
}
...
function isFunction(component: DynamicTableComponent05, methodName: any): methodName is keyof DynamicTableComponent05 {
  return typeof component[methodName as keyof DynamicTableComponent05] === 'function';
}
