...
export class AddWellInformationScreenComponent implements OnInit {
  ...
  @Input() initialMtcFiles: FileUpload[] = [];
  @Input() mtcCallback: Function = () => {};
  @Input() mtcCallbackDelete: Function = () => {};
  private _mtcFileList: File[] = [];

  @Input()
  set mtcFileList(val: File[]) {
    this._mtcFileList = val;
    console.log('✅ EL HIJO RECIBIÓ ACTUALIZACIÓN. Cantidad:', val ? val.length : 0);
  }
  get mtcFileList(): File[] {
    return this._mtcFileList;
  }
  ...
  updateInitialFile(event: any) {
    if (this.loadFilesInformation.idTypeOfRelease === 2 && this.loadFilesInformation.id === 1) {
