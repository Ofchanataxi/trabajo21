...
export async function generateExcelRUNBES(
  ...
  // const templatePath =
  //   'src/modules/field-module/infrastructure/templates/RUNBES_TEMPLATE.xlsx';
  ...
  // if (!worksheet) {
  //   throw new Error('La hoja RUNBES no fue encontrada en la plantilla.');
  // }
  ...
  console.log('dataRigTime', dataRigTime);
...
function calculateSummaryRigTime(records: any[]) {
  ...
  let fechaInicioMax: Date = records[0].fechaHoraInicio;
  let fechaFinMax: Date = records[0].fechaHoraFin;
  let duracionTotalMs = 0;
  for (const r of records) {
    if (r.fechaHoraInicio > fechaInicioMax) fechaInicioMax = r.fechaHoraInicio;
    if (r.fechaHoraFin > fechaFinMax) fechaFinMax = r.fechaHoraFin;
    duracionTotalMs += r.fechaHoraFin.getTime() - r.fechaHoraInicio.getTime();
  }
