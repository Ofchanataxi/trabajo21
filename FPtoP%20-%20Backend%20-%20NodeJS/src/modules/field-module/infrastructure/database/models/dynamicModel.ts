...
export async function createModel(tableName: string): Promise<{
  ...
  if (columns.id && !columns.id.primaryKey) {
...
"tallyGroupParent",
"tallyGroup",
case when "tallyGroupParent" is null then (select ROUND(CAST(SUM(length) AS numeric), 2) from "ElementTally" et where "tallyGroupParent" = equipmentData."tallyGroup") end  medidatotal,
"showInTally"
from
(
SELECT
...
rs.id as idestado,
ROUND(CAST(coalesce(er.availablequantity, er.quantity) AS numeric), 2) as "ElementRelease.availablequantity",
